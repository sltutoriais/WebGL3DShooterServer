...
socket.on("JOIN_ROOM",function(_data){
  ...
  socket.broadcast.emit('SPAWN_PLAYER',current_player.id,current_player.name,current_player.avatar,current_player.dx,current_player.dz);
...
socket.on("POS_AND_ROT",function(_data){
 ...
 clientLookup[current_player.id].rotation = pack.rotY;
